import { GroqTicoService } from '../../../../../src/services/tico/providers/groq.provider.js';
...
describe('GroqTicoService', () => {
  ...
  it('usa ticoTools por defecto cuando no se pasa un segundo argumento', () => {
    const service = new GroqTicoService('key');
    const tools = service.buildProviderTools();

    expect(tools.some((t) => t.function.name === 'search_motorcycles')).toBe(true);
  });

  it('buildProviderTools convierte los "type" de Gemini (mayusculas) a JSON Schema (minusculas)', () => {
    const service = new GroqTicoService('key', [fakeTool]);
  ...
  it('convierte tambien los "type" dentro de arrays (p. ej. enum)', () => {
    ...
    const service = new GroqTicoService('key', [toolWithArray]);
