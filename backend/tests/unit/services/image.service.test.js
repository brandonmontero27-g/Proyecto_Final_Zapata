import { uploadMotorcycleImages } from '../../../src/services/image.service.js';
...
describe('image.service (Supabase Storage real)', () => {
  it('sube una imagen real, la convierte a webp y devuelve una URL publica', async () => {
    const urls = await uploadMotorcycleImages('test-image-service', [TINY_PNG_BASE64]);

    expect(urls).toHaveLength(1);
    expect(urls[0]).toMatch(/\.webp$/);
    expect(urls[0]).toContain('motorcycle-images');
  });

  it('rechaza mas de 8 imagenes', async () => {
    const images = Array(9).fill(TINY_PNG_BASE64);
    await expect(uploadMotorcycleImages('test-too-many', images)).rejects.toMatchObject({ statusCode: 400 });
  });
  ...
  it('rechaza contenido que no es una imagen valida', async () => {
    ...
    await expect(uploadMotorcycleImages('test-invalid', [fakeImage])).rejects.toThrow();
  });

  it('acepta base64 plano sin el prefijo data:image/...;base64,', async () => {
    const plainBase64 = TINY_PNG_BASE64.split(',')[1];
    const urls = await uploadMotorcycleImages('test-plain-base64', [plainBase64]);
    expect(urls).toHaveLength(1);
  });
  ...
  it('rechaza una imagen que excede el limite de 5MB', async () => {
    ...
    await expect(uploadMotorcycleImages('test-too-big', [bigImage])).rejects.toMatchObject({ statusCode: 400 });
  });
  ...
  it('rechaza un formato valido para sharp pero no permitido (gif)', async () => {
    ...
    await expect(uploadMotorcycleImages('test-gif', [gifImage])).rejects.toMatchObject({ statusCode: 400 });
  });
});
